...
CALLSET_ENTRY (simple_switches, sw_right_outlane, sw_left_outlane) {
	if (in_game && !in_tilt) {
			...
			sound_start (ST_SPEECH, outlaneSoundArray[SoundChooser], SL_4S, PRI_GAME_QUICK2);
...
void simple_sounds (void) {
	...
	if ( simple_SoundCounter == 0 )			sound_start (ST_EFFECT, ZAPP_1, SL_2S, PRI_GAME_QUICK2);
	else if ( simple_SoundCounter == 1 )	sound_start (ST_EFFECT, JOINK, SL_2S, PRI_GAME_QUICK2);
	else if ( simple_SoundCounter == 2 )	sound_start (ST_EFFECT, JOINK_2, SL_2S, PRI_GAME_QUICK2);
	else if ( simple_SoundCounter == 3 )	sound_start (ST_EFFECT, ZAPP_2, SL_2S, PRI_GAME_QUICK2);
}//end of function
