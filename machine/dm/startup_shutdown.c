...
CALLSET_ENTRY (startup_shutdown, sw_launch_button) {
	if (in_game && !in_tilt && !valid_playfield ) 	sound_start (ST_ANY, EXPLOSION, SL_4S, PRI_GAME_QUICK3);
}//end of function
...
CALLSET_ENTRY (startup_shutdown, tilt) {
		if (system_config.extra_sounds == YES || system_config.swear_sounds == YES)
			sound_start (ST_EFFECT, SPCH_DONT_SLAP_ME_GIRL, SL_4S, PRI_GAME_QUICK3);
		else
			sound_start (ST_EFFECT, TILT, SL_1S, PRI_GAME_QUICK3);
//		task_sleep_sec (4);
...
void startup_effect_deff (void){
	...
	if (system_config.extra_sounds == YES || system_config.swear_sounds == YES ) {
			switch (chooser) {
				case 0: sound_start (ST_SPEECH, SPCH_SPARTANS_CHARGES_MAN, SL_4S, PRI_GAME_QUICK3); break;
				case 1: sound_start (ST_SPEECH, SPCH_ELIGIBLE_PAROLE_MAN, SL_4S, PRI_GAME_QUICK3); 	break;
				case 2: sound_start (ST_SPEECH, SPCH_PLAY_BALL_SIMON, SL_4S, PRI_GAME_QUICK3); 		break;
			}//end of switch
	} else	sound_start (ST_SPEECH, SPCH_DEMOLITION_MAN, SL_4S, PRI_GAME_QUICK3);
	...
	if ((system_config.extra_sounds == YES || system_config.swear_sounds == YES) && chooser ) sound_start (ST_SPEECH, SPCH_DEMOLITION_MAN, SL_4S, PRI_GAME_QUICK3);
	deff_exit ();
}//end of function
