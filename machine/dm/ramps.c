...
void lramp_arrow_light_on(void) {
	...
	if (get_capture_simon_arrows (CAPSIM_LEFTRAMP)) 	lamp_tristate_flash (LM_LEFT_RAMP_ARROW);
	else 												lamp_tristate_on (LM_LEFT_RAMP_ARROW);
}//end of function
...
void rramp_arrow_light_on(void) {
	...
	if (get_capture_simon_arrows (CAPSIM_RIGHTRAMP)) 	lamp_tristate_flash (LM_RIGHT_RAMP_ARROW);
	else 												lamp_tristate_on (LM_RIGHT_RAMP_ARROW);
}//end of function
...
void cramp_arrow_light_on(void) {
	...
	if (get_capture_simon_arrows (CAPSIM_CENTERRAMP)) 	lamp_tristate_flash (LM_CENTER_RAMP_ARROW);
	else 												lamp_tristate_on (LM_CENTER_RAMP_ARROW);
}//end of function
...
void sramp_arrow_light_on(void) {
	...
	if (get_capture_simon_arrows (CAPSIM_SIDERAMP)) 	lamp_tristate_flash (LM_SIDE_RAMP_ARROW);
	else 												lamp_tristate_on (LM_SIDE_RAMP_ARROW);
}//end of function
...
void right_ramp_made(void) {
	...
	else if (get_jackpot_arrows(rramp_mask) ) 				score_jackpot();
	if (is_huxley_running ()  )								huxley_mode_shot_made();
	if (is_prison_break_running () ) 						prison_break_made();
...
void ramp_sounds (void) {
	...
	if (++rampSoundsCounter % 3 == 0)
						sound_start (ST_SPEECH, rampSoundsArray[rand_sound], SL_4S, PRI_GAME_QUICK2);
	else if (rampSoundsCounter % 3 == 1)
						sound_start (ST_ANY,explosionSoundsArray[random_scaled(4)], SL_4S, PRI_GAME_QUICK2);
}//end of function
