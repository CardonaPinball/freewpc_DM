 ...
 *
 */
/* CALLSET_SECTION (acmag, __machine4__) */
...
void acmag_start_effect_deff(void) {
	...
	dmd_draw_thin_border (dmd_low_buffer);
		font_render_string_center (&font_halobold12, DMD_MIDDLE_X, DMD_BIG_CY_Top, "ACMAG");
...
void acmag_effect_deff(void) {
	...
	for (;;) {
		...
		if (ballsave_test_active()) {
			sprintf ("%d", ballsave_get_timer() );
			font_render_string_center (&font_var5, 6, 6, sprintf_buffer);
		}

		sprintf_score(current_score);
		font_render_string_center (&font_var5, DMD_MIDDLE_X, DMD_SMALL_CY_1-1, sprintf_buffer);
			font_render_string_center (&font_halobold12, DMD_MIDDLE_X, DMD_BIG_CY_Top+4, "ACMAG");
		...
		if (i % 20 != 0) { //draw for 4/5 and blank for 1/5
					if (TOGGLE) {
						...
						font_render_string_center (&font_var5, DMD_MIDDLE_X, DMD_SMALL_CY_3 + 5, sprintf_buffer);
							}//end of if (TOGGLE)
					else {
		...
		if (TOGGLE_BOTTOM)	font_render_string_center (&font_var5, DMD_MIDDLE_X, DMD_MED_CY_BOTTOM_LINE + 3, "SHOOT CENTER RAMP");
		else			{	sprintf_score (acmag_mode_next_score);
							font_render_string_center (&font_var5, DMD_MIDDLE_X, DMD_MED_CY_BOTTOM_LINE + 3, sprintf_buffer); }
