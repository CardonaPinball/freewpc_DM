...
void jets_effect_deff(void) {
	...
	if (jets_modes_achieved % 2 == 0) { //DISPLAY THIS ON EVEN NUMBERED JETS MODES
		...
		do {
			...
			font_render_string_center (&font_fixed6, DMD_MIDDLE_X - 20, DMD_MED_CY_BOTTOM_LINE, sprintf_buffer);
			dmd_show_low ();
		...
		do {
			...
			sprintf_score (jets_temp_score);
			font_render_string_center (&font_term6, DMD_MIDDLE_X + 20, DMD_MED_CY_BOTTOM_LINE, sprintf_buffer);
