...
void lamplist_apply (lamplist_id_t id, lamp_operator_t op) {
	...
	for (entry = lamplist_table[id]; *entry != LAMP_END; entry++) {
		switch (*entry) {
			case LAMP_BREAK:
				...
				if (lamplist_apply_delay1) lamplist_leff_sleep (lamplist_apply_delay1);
				break;
	...
	if (lamplist_apply_delay1 != 0) lamplist_apply_delay = lamplist_apply_delay1;
	page_pop ();
}
...
/** Apply an operator to each element of a lamplist, one by one.
 *  when the next set is turned changed, the previous is changed also.
 *  Macros are executed as they are encountered. */
void lamplist_apply_step (lamplist_id_t id, lamp_operator_t op) {
	register const lampnum_t *entry;
	U8 lamplist_apply_delay1 = 0;
	U8 count;
	count = 0;
	...
	for (entry = lamplist_table[id]; *entry != LAMP_END; entry++) {
		switch (*entry) {
			...
			case LAMP_BREAK:
				...
				if (lamplist_apply_delay1) lamplist_leff_sleep (lamplist_apply_delay1);
				break;
			//if not a break then perform action on next lamps
			default:
				count++;
				(*op) (*entry);
				lamplist_leff_sleep (lamplist_apply_delay);
				if (count > 2) 	(*op) (*entry - 2); //swap lamps 2 back
				break;
		}//end of switch
	}//end of loop
...
static const lampnum_t *lamplist_find (lamplist_id_t id, lamp_boolean_operator_t op) {
	register const lampnum_t *entry;
...
static __pure__ lamp_boolean_operator_t matrix_test_operator (const lamp_set matrix) {
