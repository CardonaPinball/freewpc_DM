...
void high_score_reset (void) {
	...
	for (place=0; place < NUM_HIGH_SCORES; place++) {
		memcpy (high_score_table[place+1].score, default_highest_scores[place], HIGH_SCORE_WIDTH);
		memcpy (high_score_table[place+1].initials, default_high_score_initials[place], HIGH_SCORE_NAMESZ);
	}
...
void hsentry_deff (void) {
	dmd_alloc_low_clean ();
	switch (high_score_player) {
		case 1: sound_start (ST_SPEECH, SPCH_PLAYER_ONE, SL_4S, PRI_GAME_QUICK3);  break;
		case 2: sound_start (ST_SPEECH, SPCH_PLAYER_TWO, SL_4S, PRI_GAME_QUICK3);  break;
		case 3: sound_start (ST_SPEECH, SPCH_PLAYER_THREE, SL_4S, PRI_GAME_QUICK3);  break;
		case 4: sound_start (ST_SPEECH, SPCH_PLAYER_FOUR, SL_4S, PRI_GAME_QUICK3);  break;
	}
...
void high_score_free (U8 position) {
	//verify not last score, if not then recurse until at last score
	if (position < HS_COUNT-1) high_score_free (position+1);

	//copy score from position above to position below
	memcpy (&high_score_table[position+1], &high_score_table[position], sizeof (struct high_score));
}
...
void high_score_check_player (U8 player)
{
	...
	for (hs = 0; hs < HS_COUNT; hs++) { //loop through all scores starting at GC and going down
		struct high_score *hsp = &high_score_table[hs];
		if ( (score_compare (scores[player], hsp->score)) > 0 ) {
			// Push all scores down and then insert the player score here.
			// Set the initials to the player number
			pinio_nvram_unlock ();

			high_score_free (hs); //push all the scores down by one
...
void high_score_enter_initials (U8 position) {
	...
	if (hsp->initials[0] <= MAX_PLAYERS) {
		...
		if (system_config.disable_knocker == NO) {
				U8 count;
				count = 0;
...
//this is the first function called at end game to start the whole high score entry
//
// Check to see if the current player score qualifies for the high score board.
void high_score_check (void) {
	...
	for (player = 0; player < num_players; player++) high_score_check_player (player);

	// Now that all high scores have been entered, scan the table to see which initials need to be entered
