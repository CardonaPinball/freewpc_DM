...
//		if (ch->timer == 0 || sound_start_prio >= ch->prio)
		if (	ch->timer == 0
			|| 	(sound_start_prio > ch->prio  && channels == ST_SPEECH) //if speech, only preempt if a higher priority
			|| 	(sound_start_prio >= ch->prio  ) ) {//if effects, can preempt if same priority
			/* If a duration was given, then reserve the channel until it is done. */
