...
done. This takes into account both the balls in the trough and any balls that are
in any container device such as a ball lock.*/
U8 counted_balls;
...
//	task_sleep (dev->props->settle_delay); //this defaults to 700ms
...
start_update:
		...
		if (unlikely (dev->actual_count >= dev->previous_count)) {
			...
			if (dev->kick_errors == (trough_dev_p (dev) ? 20 : 7)) {
				...
				if (dev->kick_errors <= 3)
					task_sleep_sec (2); //ORIGINAL WAS 1 SEC
				else
					task_sleep_sec (5);//ORIGINAL WAS 5 SEC
			}
		}
		else if (dev->actual_count < dev->previous_count) {
	...
	if (dev->kicks_needed > 0) {
		if (dev->actual_count == 0) {
			...
			if (dev->state == DEV_STATE_IDLE)  dev->state = DEV_STATE_RELEASING;
...
void device_zero_out_kick_request (device_t *dev) {
			dev->kicks_needed = 0;
}
...
void device_request_kick (device_t *dev) {
	...
	if (device_kickable_count (dev) > 0) {
		...
		if (!trough_dev_p (dev)) live_balls++;
	}
...
void device_request_empty (device_t *dev) {
	...
	/* TODO - this logic probably belongs somewhere else */
//	if (!trough_dev_p (dev))	live_balls += can_kick;
 ...
 * any time a change happens locally within a particular device. */
void device_update_globals (void)
{
	...
	for (devno = 0; devno < NUM_DEVICES; devno++) {
		...
		if (trough_dev_p (dev) ) balls_in_trough = dev->actual_count;
	}
	...
	if (missing_balls > 0xF0) missing_balls = 0;
...
void device_add_live (void) {
	if (missing_balls > 0)  missing_balls--;
	live_balls++;
}
...
/*
 * in the old logic we were relying on callset_invoke (ball_drain)
 * to return false to determine if the ball drain was valid or not.
 * since the calls went out to approximately 14 different areas in the code
 * there was a long delay here.  as a work around, I incorporated most of
 * that code here using extern vars - this increases the speed quite well
 * but is obviously a proprietary solution.
 * the callset_invoke (ball_drain) still go out to check if the coin door
 * is open or if we are in normal ball save - so there is yet 2 calls.
 */
extern __boolean	wasteland_ballsave;
extern __boolean	is_wasteland_running;
extern __boolean	museum_ballsave;
extern __boolean	is_museum_running;
extern __boolean	is_fortress_running;
extern __boolean	fortress_ballsave;
extern __boolean	is_explode_running;
extern __boolean	explode_ballsave;
extern __boolean	demotime_ballsave;
extern __boolean	is_demotime_running;
extern __boolean	cryoprison_ballsave;
extern __boolean	is_cryoprison_running;
extern __boolean 	carcrash_ballsave;
extern __boolean	is_carchase_running;
extern __boolean	is_back_in_the_fridge_in_ball_save;

__boolean dv_ball_drain(void) {
	if (!in_tilt) {
			if	(carcrash_ballsave)	{ //this one currently does not work
				serve_ball_auto ();
				return FALSE; //this is not a valid drain, don't count it
			}
			else
			if	(is_back_in_the_fridge_in_ball_save)	{
				sound_start (ST_EFFECT, LOW_HORN, SL_1S, PRI_GAME_QUICK4);
				serve_ball_auto ();
				return FALSE;
			}
			else
			if	( 	(is_wasteland_running && wasteland_ballsave) ||
					(is_fortress_running && fortress_ballsave) ) {
				sound_start (ST_SPEECH, SPCH_AHHHGGG, SL_2S, PRI_GAME_QUICK2);
				serve_ball_auto ();
				return FALSE; //this is not a valid drain, don't count it
			}
			else
			if	(is_museum_running && museum_ballsave)	{
				sound_start (ST_SPEECH, SPCH_UHHN, SL_4S, PRI_GAME_QUICK2);
				serve_ball_auto ();
				return FALSE; //this is not a valid drain, don't count it
			}
			else
			if	(	(is_explode_running && explode_ballsave) ||
					(is_demotime_running && demotime_ballsave) ) {
				sound_start (ST_SAMPLE, EXPLOSION, SL_2S, PRI_GAME_QUICK1);
				serve_ball_auto ();
				return FALSE; //this is not a valid drain, don't count it
			}
			else
			if	(is_cryoprison_running && cryoprison_ballsave)	{
				sound_start (ST_SPEECH, SPCH_JOHN_SCREAM, SL_2S, PRI_GAME_QUICK2);
				serve_ball_auto ();
				return FALSE; //this is not a valid drain, don't count it
			}
...
void device_remove_live (void) {
	...
	//	callset_invoke (missing_ball_found);
		missing_balls = 5 - device_entry (DEVNO_TROUGH)->actual_count;
	}
	if (live_balls > 0)
		/* Decrement the count of balls in play */
		live_balls = 5 - device_entry (DEVNO_TROUGH)->actual_count;
	...
	if (in_game && !in_bonus) {
			...
			back into play. */
			if (!dv_ball_drain() ) return; //placed this here to remove huge call stack and make faster
			if (!callset_invoke_boolean (ball_drain)) return;//legacy support

			/* OK, at this point, it is a true ball drain event.
			See how many balls are in play now. */
			switch (live_balls + device_entry (DEVNO_TROUGH)->kicks_needed) {
				case 0:
				...
				case 1:
					...
					break;
				default: //if more than one ball was in play, do nothing here
					break;
			}
	}//end of 	if (in_game && !in_bonus)
}//end of function
...
void device_lock_ball (device_t *dev)
{
	...
	if (device_kickable_count (dev)) live_balls--;
...
void device_init (void) {
	...
	balls_in_trough = device_recount(device_entry (DEVNO_TROUGH) );
}
