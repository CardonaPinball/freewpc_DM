...
void window_push (struct window_ops *ops, void *priv)
{
	...
	if (win_top == NULL) {
		//flipper_enable ();
		if (!in_test) window_push_first ();
		else task_kill_gid (GID_DIAG_RUNNING);
		win_top = &win_stack[0];
	}
...
void window_pop_quiet (void) {
	...
	if (win_top == &win_stack[0]) {
		...
		win_top--;
	//	if (win_top == &win_stack[0]) flipper_enable ();
		window_start_thread ();
		window_redraw ();
	}
}
...
char font_test_alphabet[] = "DEMOLITION0123456789";
...
void font_test_draw (void)
{
	...
	if (font_height < 8)
	{
		...
		task_runs_long ();
		if (font_test_offset < 20) //print a second line
		{
...
//	SOL_DUTY_12, SOL_DUTY_25, SOL_DUTY_37, SOL_DUTY_50, SOL_DUTY_62, SOL_DUTY_75, SOL_DUTY_100
...
bool solenoid_test_selection_ok (void) {
	//verify that it is not a flipper - if so, skip over it
	if ((menu_selection >= 28) && (menu_selection <= 35))
		return 0;
	browser_action = sol_get_time (menu_selection);

	U8 is_flasher = MACHINE_SOL_FLASHERP (menu_selection);
	//if it is a flasher and we are in flasher mode then return true
	//if it is not a flasher and we are not in flasher mode then return true
	if (is_flasher == flasher_test_mode) return 1;
	else return 0;
}



// turn the constant into an index - why?
void driver_update_duty (void) {
	sol_duty_level = 3; //SOL_DUTY_100
...
void solenoid_test_draw (void) {
	...
	if (browser_action == sol_get_time (win_top->w_class.menu.selected))
					font_render_string_left (&font_var5, 35, 10, "TIME");
	...
	if (sol_duty_masks[sol_duty_level] == sol_get_duty (win_top->w_class.menu.selected))
			font_render_string_right (&font_var5, 100, 10, "PWR");
...
void solenoid_test_up (void) {
	do {
		sound_send (SND_TEST_UP);
		menu_selection++;
		if (menu_selection >= 15) menu_selection = 15;
	} while (!solenoid_test_selection_ok ());
	driver_update_duty ();
}
...
void flasher_test_up (void) {
	do {
		sound_send (SND_TEST_UP);
		task_sleep (TIME_500MS);
		menu_selection++;
		if (menu_selection >= 47) menu_selection = 47;
	} while (!solenoid_test_selection_ok ());
	driver_update_duty ();
}
...
void flasher_test_down (void) {
	do {
		sound_send (SND_TEST_DOWN);
		if (menu_selection <= 16) menu_selection = 16;
		else menu_selection--;
	} while (!solenoid_test_selection_ok ());
	driver_update_duty ();
}
...
void test_up_button (void) {
	...
	for (i=0; i < 16; i++)
		if (!switch_poll (SW_UP)) return;
		else task_sleep (TIME_33MS);
